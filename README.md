...
AEGIS is something new: the first distributed immune system for agents, organized much like a biological one. It is designed to be a highly robust crowdsourced network for protection against prompt injections of all types, but especially against prompt worms that self-replicate in shared multi-agent spaces such as Moltbook. It can also be used to adaptively block universal jailbreaks after the first few harmful actions taken using a given jailbreak, compressing a patch loop that would have previously required manual review. AEGIS drops-in with a single line of code and is secure out of the box, but is also highly configurable and modular - read on for how to tune it with global and setting-specific configs. AEGIS detects prompt injections, contains compromised agents, and prevents cascading attacks across multi-agent systems so you don't end up inadvertently authoring the first chapter of a sci-fi novel.

AEGIS implements our [Semantic Immunity](https://github.com/gaiarobotics/papers/blob/main/Semantic%20Immunity%20Paper.pdf) framework. The adaptive component of AEGIS maintains a shared database of compromised agent embedding hashes. Compromised agents themselves will populate it through the innate component of the framework and existing adaptive population immunity. This results in true immunological memory that scales automatically with the strength of the models using the system, without introducing another LLM layer that could itself be vulnerable to prompt injection.
